...
pub async fn billing_scheduler(appstate: web::Data<AppState>,) -> Result<String, anyhow::Error> {
    ...
    let wallet: LocalWallet = appstate.operator_key.parse().context("Unable to parse operator private key")?;

    let client = SignerMiddleware::new(provider, wallet);
    let contract = Contract::new(appstate.contract.as_str()
                                                                                        .parse::<Address>()
    ...
    let (rs, v) = appstate.signer.sign_prehash_recoverable(&hash).context("failed to sign requests")?;
    let signature = hex::encode(rs.to_bytes().append(27 + v.to_byte()).as_slice());
    
    let tx = contract.method::<_, H256>("settle", (tx_hashes, amounts, signature)).context("failed to build transaction request for billing")?;
    ...
    Ok(receipt.ok_or(anyhow!("Failed to parse transaction receipt!"))?.transaction_hash.to_string())
}